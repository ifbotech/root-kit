 ...
 *
 * Acá viven las cuatro conversiones que hacen que el Prime y los Minis se
 ...
 *
 * El ánimo se evalúa UNA vez, en el nodo dueño de la maceta, y viaja
 * resuelto. `rk_link_ingest` lo copia tal cual al roster del Prime y NO
 * vuelve a llamar a rk_mood_eval sobre esos datos. Si algún día alguien
 * agrega esa llamada "por las dudas", el Prime y el Mini pueden discrepar
 * —distinta histéresis acumulada, distinto conteo de muestras oscuras— y el
 * usuario ve una cara en la maceta y otra en el escritorio sin forma de
 * saber cuál le miente. Hay un test que fija este comportamiento.
 */
 ...
 * si falta el nodo o la especie —un nodo sin especie no puede evaluarse solo
 * y no tiene sentido configurarlo todavía. */
 ...
 * NULL si el id no está en el kit o si la secuencia es vieja. `now_s` es el
 * reloj del Prime, y de él sale la antigüedad que después decide el enlace. */
...
/* Reconstruye del paquete de configuración la especie con la que el Mini va
 * a evaluarse. `sp_out` es memoria del llamador: los umbrales viajan, los
...
/* Arma la telemetría que el Mini emite, con su ánimo ya evaluado. */
