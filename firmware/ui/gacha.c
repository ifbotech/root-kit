...
{
    rk_rarity_t rar = rk_rarity_of(c);
    rk_color_t  tono = rk_rarity_color(rar);
    int cx = RK_CANVAS_W / 2;
    int cy = 108;
    int i;
     ...
     * anticipa el premio sin mostrarlo todavía. */
    {
        ...
        rk_vgradient(fb, 0, 0, RK_CANVAS_W, RK_CANVAS_H,
    ...
    if (t_ms < RK_GACHA_CAIDA) {
        ...
        capsula(fb, cx, cy - caida - rebote, 34, 0, tono);
        ...
        grietas(fb, cx + jx, cy, 34, (int)p, tono, t_ms);
        capsula(fb, cx + jx, cy, 34, (int)p / 30, tono);
        ...
        {
            int sep = (int)p * 3;
            capsula(fb, cx - sep / 2, cy - sep, 30, 0, tono);
            capsula(fb, cx + sep / 2, cy + sep, 30, 0, tono);
        }
        ...
        if (p > 20u) {
            ...
            rk_tuga_draw(fb, cx, cy + sube, RK_MOOD_HAPPY, RK_SEV_OK, t_ms);
        }

    } else {
        /* Reposo: el bicho, su nombre, su rareza y su lema. */
        int halo;
        for (halo = 46; halo > 30; halo -= 4) {
            rk_color_t h = rk_mix(C_FONDO, tono,
                                  (uint8_t)(30 - (halo - 30) * 2));
        ...
        for (i = 0; i < rk_rarity_sparkles(rar) / 6; i++) {
            ...
            int x = cx - 60 + (int)(h % 120u);
            int y = 60 + (int)((h >> 5) % 90u);
            y += rk_sin8((uint8_t)(t_ms / 18u + i * 40u)) * 4 / 127;
            rk_px(fb, x, y, tono);
        }

        rk_tuga_draw(fb, cx, cy, RK_MOOD_HAPPY, RK_SEV_OK, t_ms);

        /* Cartel de rareza. */
        {
            const char *rn = rk_rarity_name(rar);
            int w = rk_text_w(rn, 1) + 12;
            rk_fill_rect(fb, cx - w / 2, 154, w, 13, rk_mix(C_FONDO, tono, 45));
            rk_rect(fb, cx - w / 2, 154, w, 13, tono);
            rk_text_center(fb, cx, 157, rn, tono, 1);
        }

        rk_text_center(fb, cx, 174, c ? c->nombre : "???", C_INK, 2);
        ...
        if (c != NULL && c->lema != NULL) {
            ...
            if (corte < 0 || rk_text_w(s, 1) <= RK_CANVAS_W - 12) {
                rk_text_center(fb, cx, 196, s, C_INK_DIM, 1);
            } else {
                char a[40], b[40];
                ...
                b[j] = '\0';
                rk_text_center(fb, cx, 194, a, C_INK_DIM, 1);
                rk_text_center(fb, cx, 204, b, C_INK_DIM, 1);
            }
        }

        if (rk_gacha_termino(t_ms) && ((t_ms / 600u) % 2u) == 0u) {
            rk_text_center(fb, cx, 224, "TOCA PARA CONTINUAR", C_INK_DIM, 1);
        }
    }

    /* Encabezado, siempre presente: dice qué está pasando. */
    rk_fill_rect(fb, 0, 0, RK_CANVAS_W, 22, rk_mix(C_FONDO, 0x0000, 90));
