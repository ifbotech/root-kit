...
/* Debajo de esto la pila pide cambio. Mismo umbral que ui/cara.c, que es
 * donde el aparato lo dibuja. */
const BATT_AVISO = 15;
...
export function tareasDe(nodo, especie) {
  ...
  const n = nodo;
  const tel = n.tel || {};
  const nombre = n.nombre || 'esta maceta';
  ...
  if (n.link === 'CAIDO') {
    add('revisar', conNombre('Revisar el aparato de', nombre),
      'Hace horas que no manda datos. Puede ser la pila, el wifi, o que se haya soltado.',
      'pronto', 'antena', false);
  }

  const batt = n.nodo?.batt_pct;
  if (Number.isFinite(batt) && batt < BATT_AVISO) {
    add('pila', conNombre('Cambiar la pila de', nombre),
      `Le queda ${batt}%. Cuando se apague dejás de enterarte de cómo está la planta.`,
      batt < BATT_CRITICA ? 'urgente' : 'pronto', 'pila', false);
  }

  /* Altas a medio terminar. Son tareas de verdad: sin especie el aparato no
   * sabe con qué umbrales juzgar, y sin carcasa no sabe qué cara poner. */
  if (!n.especie) {
    add('especie', conNombre('Terminar el registro de', nombre),
      'Sin saber qué planta es, el aparato no puede decidir si está bien o mal.',
      'pronto', 'lupa', false);
  } else if (!n.modelo) {
    add('carcasa', `Decir qué carcasa le tocó a ${nombre}`,
      'El aparato mide igual, pero no sabe qué cara poner hasta que se lo digas.',
      'cuando-puedas', 'caja', false);
  }
