...
describe('tareas del día', () => {
  ...
  test('un alta a medias es una tarea', () => {
    ...
    const sinCarcasa = tareasDe(nodo({ modelo: null }), MONSTERA);
    assert.ok(sinCarcasa.some((x) => x.tipo === 'carcasa'));
  });
  ...
  test('las tareas que el sensor confirma se marcan como automáticas', () => {
    ...
    const pila = tareasDe(nodo({ nodo: { batt_pct: 5 } }), MONSTERA)
      .find((t) => t.tipo === 'pila');
    assert.equal(pila.auto, false, 'cambiar la pila no lo confirma solo');
  });
