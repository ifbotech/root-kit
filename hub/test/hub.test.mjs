...
describe('API', () => {
  ...
  test('rechaza altas incompletas o con especie inventada', async () => {
    assert.equal((await manejarApi('POST', '/api/nodes', { nombre: 'X' }))[0], 400);
    assert.equal((await manejarApi('POST', '/api/nodes', { especie: 'monstera' }))[0], 400);
    assert.equal((await manejarApi('POST', '/api/nodes',
  ...
  test('asignar un Spore lo saca de los libres', async () => {
    const [, libresAntes] = await manejarApi('GET', '/api/nodos', null);
    assert.equal(libresAntes.length, 1);
    await manejarApi('POST', '/api/nodes',
      { nombre: 'NUEVA', especie: 'pothos', nodo_id: 'a4cf129b40aa' });
    const [, libresDespues] = await manejarApi('GET', '/api/nodos', null);
    assert.equal(libresDespues.length, 0);
  });

  test('editar y borrar una planta', async () => {
    const [, p] = await manejarApi('PATCH', '/api/nodes/p1', { nombre: 'RENOMBRADA' });
    assert.equal(p.nombre, 'RENOMBRADA');
    assert.equal((await manejarApi('PATCH', '/api/nodes/p1', { especie: 'nope' }))[0], 400);
    assert.equal((await manejarApi('DELETE', '/api/nodes/p1', null))[0], 204);
  ...
  test('las rutas desconocidas dan 404 en vez de romper', async () => {
    ...
    assert.equal((await manejarApi('GET', '/api/nodes/noexiste', null))[0], 404);
  });
});
